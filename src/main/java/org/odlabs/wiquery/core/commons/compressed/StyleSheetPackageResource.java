...
public class StyleSheetPackageResource extends CompressedPackageResource {
	private static final long serialVersionUID = 1L;;

	private static final Logger log = LoggerFactory
			.getLogger(StyleSheetPackageResource.class);
	 ...
	 */
	public static final HeaderContributor getHeaderContribution(
			final Class<?> scope, final String path) {
	 ...
	 * rewrite.</li>
	 * <li>Starts with anything else, which Wicket will automatically prepend to
	 * make relative to the context root of your web-app.</li>
	 * </ul>
	 * 
	 * @param location
	 ...
	 * @param style
	 *            The style of the resource (see
	 *            {@link org.apache.wicket.Session})
	 * @return The resource
	 */
	public static PackageResource newPackageResource(final Class<?> scope,
			final String path, final Locale locale, final String style) {
		return new StyleSheetPackageResource(scope, path, locale, style);
	}
	 ...
	 * @param style
	 */
	protected StyleSheetPackageResource(Class<?> scope, String path,
			...
			protected byte[] filterContent(byte[] input) {
				try {
					...
					if (Application.get().getResourceSettings()
						...
						return JavascriptStripper.stripCommentsAndWhitespace(s)
								.getBytes("UTF-8");
					}
