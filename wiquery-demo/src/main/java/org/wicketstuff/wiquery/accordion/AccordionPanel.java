...
public class AccordionPanel extends Panel
{
	...
	public AccordionPanel(String id)
	{
		...
		JsStatement getActiveStatement = new JsQuery(accordion).$().chain("accordion", "'option'",
			"'active'");
		CharSequence labelValue = "'Active is: ' + " + getActiveStatement.render(false);
		accordion.setActivateEvent(
			JsScopeUiEvent.quickScope(new JsQuery(indexLabel).$().chain("text", labelValue)));
	}
}
